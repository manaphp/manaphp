...
class DataDbQueryTest extends TestCase
{
    public function setUp()
    {
        ...
        $db->attachEvent(
            'db:beforeQuery', function (DbInterface $source, $data) {
    ...
    public function test_select()
    {
        ...
        $this->assertEquals(
            'SELECT [city_id] AS [id], [city_name] FROM [city]',
    ...
    {
        $this->assertEquals(
            'SELECT * FROM [city] LEFT JOIN [country] ON [country].[city_id]=[city].[city_id]',
        ...
        $this->assertEquals(
            'SELECT * FROM [city] AS [c1] LEFT JOIN [country] AS [c2] ON [c2].[city_id]=[c1].[city_id]',
    ...
    {
        $this->assertEquals(
            'SELECT * FROM [city] INNER JOIN [country] ON [country].[city_id]=[city].[city_id]',
        ...
        $this->assertEquals(
            'SELECT * FROM [city] AS [c1] INNER JOIN [country] AS [c2] ON [c2].[city_id]=[c1].[city_id]',
    ...
    {
        $this->assertEquals(
            'SELECT * FROM [city] RIGHT JOIN [country] ON [country].[city_id]=[city].[city_id]',
        ...
        $this->assertEquals(
            'SELECT * FROM [city] AS [c1] RIGHT JOIN [country] AS [c2] ON [c2].[city_id]=[c1].[city_id]',
    ...
    public function test_where()
    {
        ...
        $this->assertEquals(
            'SELECT * FROM [city] WHERE [c].[city_id]=:c_city_id',
    ...
    {
        $this->assertEquals(
            'SELECT * FROM [city] WHERE FIND_IN_SET(:city_id, [city_id])>0',
    ...
    {
        $this->assertEquals(
            'SELECT * FROM [city] WHERE [city_id] BETWEEN :city_id_min AND :city_id_max',
            (new Query())->from('city')->whereBetween('city_id', 1, 10)->getSql()
        );

        $this->assertEquals(
            'SELECT * FROM [city] WHERE [c].[city_id] BETWEEN :c_city_id_min AND :c_city_id_max',
    ...
    {
        $this->assertEquals(
            'SELECT * FROM [city] WHERE [city_id] NOT BETWEEN :_min_0 AND :_max_0',
            (new Query())->from('city')->whereNotBetween('city_id', 1, 10)->getSql()
        );

        $this->assertEquals(
            'SELECT * FROM [city] WHERE [c].[city_id] NOT BETWEEN :_min_0 AND :_max_0',
    ...
    public function test_whereIn()
    {
        ...
        $this->assertEquals(
            'SELECT * FROM [city] WHERE [city_id] IN (1)',
            (new Query())->from('city')->whereIn('city_id', [1])->getSql()
        );
        $this->assertEquals(
            'SELECT * FROM [city] WHERE [city_id] IN (:_in_0_0)',
            (new Query())->from('city')->whereIn('city_id', ['1'])->getSql()
        );

        $this->assertEquals(
            'SELECT * FROM [city] WHERE [city_id] IN (1, 2)',
            (new Query())->from('city')->whereIn('city_id', [1, 2])->getSql()
        );
        $this->assertEquals(
            'SELECT * FROM [city] WHERE [city_id] IN (:_in_0_0, :_in_0_1)',
            (new Query())->from('city')->whereIn('city_id', ['1', '2'])->getSql()
        );
    }
    ...
    public function test_whereNotIn()
    {
        ...
        $this->assertEquals(
            'SELECT * FROM [city] WHERE [city_id] NOT IN (1)',
            (new Query())->from('city')->whereNotIn('city_id', [1])->getSql()
        );
        $this->assertEquals(
            'SELECT * FROM [city] WHERE [city_id] NOT IN (:_in_0_0)',
            (new Query())->from('city')->whereNotIn('city_id', ['1'])->getSql()
        );

        $this->assertEquals(
            'SELECT * FROM [city] WHERE [city_id] NOT IN (1, 2)',
            (new Query())->from('city')->whereNotIn('city_id', [1, 2])->getSql()
        );
        $this->assertEquals(
            'SELECT * FROM [city] WHERE [city_id] NOT IN (:_in_0_0, :_in_0_1)',
            (new Query())->from('city')->whereNotIn('city_id', ['1', '2'])->getSql()
        );

        $this->assertEquals(
            'SELECT * FROM [city] WHERE DATE(created_time) NOT IN (2000, 2001)',
            (new Query())->from('city')->whereNotIn('DATE(created_time)', [2000, 2001])->getSql()
        );
        $this->assertEquals(
            'SELECT * FROM [city] WHERE DATE(created_time) NOT IN (:_in_0_0, :_in_0_1)',
            (new Query())->from('city')->whereNotIn('DATE(created_time)', ['2000', '2001'])->getSql()
        );
    }
    ...
    {
        $this->assertEquals(
            'SELECT * FROM [city] WHERE [city_name] LIKE :city_name',
            (new Query())->from('city')->whereContains('city_name', 'A')->getSql()
        );

        $this->assertEquals(
            'SELECT * FROM [city] AS [c] WHERE [c].[city_name] LIKE :c_city_name',
            (new Query())->from('city', 'c')->whereContains('c.city_name', 'A')->getSql()
        );

        $this->assertEquals(
            'SELECT * FROM [city] WHERE ([city_name] LIKE :city_name OR [country_name] LIKE :country_name)',
    ...
    public function test_where1v1()
    {
        ...
        $this->assertEquals(
            'SELECT * FROM [city] WHERE (city_id=:city_id OR country_id=:country_id)', $query->getSql()
        );
        $this->assertEquals(['city_id' => '10', 'country_id' => '10'], $query->getBind());

        $query = (new Query())->from('city')->where1v1('city_id,country_id', '10,20');
        $this->assertEquals(
            'SELECT * FROM [city] WHERE ((city_id=:city_id_a AND country_id=:country_id_b) OR (city_id=:city_id_b AND country_id=:country_id_a))',
            $query->getSql()
        );
        $this->assertEquals(
            ['city_id_a' => '10', 'country_id_b' => '20', 'city_id_b' => '20', 'country_id_a' => '10'],
            $query->getBind()
        );
    }
    ...
    public function test_orderBy()
    {
        ...
        $this->assertEquals(
            'SELECT * FROM [city] ORDER BY [city_id] ASC, [city_name] DESC',
    ...
    {
        $this->assertEquals(
            'SELECT COUNT(*), [city_id] FROM [country] GROUP BY [city_id] HAVING COUNT(*) >10',
            (new Query())->select(['COUNT(*)', 'city_id'])->from('country')->groupBy('city_id')->having('COUNT(*) >10')
                ->getSql()
        );

        $this->assertEquals(
            'SELECT COUNT(*), [city_id] FROM [country] GROUP BY [city_id] HAVING COUNT(*) >10',
            (new Query())->select(['COUNT(*)', 'city_id'])->from('country')->groupBy('city_id')->having(
                ['COUNT(*) >10']
            )->getSql()
        );

        $this->assertEquals(
            'SELECT COUNT(*), [city_id] FROM [country] GROUP BY [city_id] HAVING (COUNT(*) >10) AND (city_id >10)',
            (new Query())->select(['COUNT(*)', 'city_id'])->from('country')->groupBy('city_id')->having(
                ['COUNT(*) >10', 'city_id >10']
            )->getSql()
        );
    ...
    {
        $this->assertEquals(
            'SELECT * FROM [city] GROUP BY [city_id]',
            (new Query())->from('city')->groupBy('city_id')->getSql()
        );

        $this->assertEquals(
            'SELECT * FROM [city] GROUP BY DATE(create_time)',
            (new Query())->from('city')->groupBy('DATE(create_time)')->getSql()
        );

        $this->assertEquals(
            'SELECT * FROM [city] GROUP BY [city_id], [city_name]',
        ...
        $this->assertEquals(
            'SELECT * FROM [city] GROUP BY [c].[city_id]',
