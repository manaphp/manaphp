    ...
    protected function _send($message, &$failedRecipients = null)
    {
        ...
        foreach (array_merge($to, $cc, $bcc) as $k => $v) {
            ...
            if ($code !== 250) {
                ...
                $this->logger->info(['Failed Recipient To <:address>: :msg', 'address' => $address, 'msg' => $msg],
                    'mailer.send');
            } else {
                $success++;
            }
        ...
        if (!$success) {
            $this->logger->info(['Send Failed:', array_merge($message->getTo(), $message->getCc(), $message->getBcc())],
                'mailer.send');
            return $success;
        }
