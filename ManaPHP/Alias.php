...
class Alias extends Component implements AliasInterface
{
    ...
    {
        if ($name[0] !== '@') {
            throw new InvalidFormatException(['`:name` must start with `@`'/**m02b52e71dba71561a*/, 'name' => $name]);
        }
    ...
    public function get($name = null)
    {
        ...
        if ($name[0] !== '@') {
            throw new InvalidFormatException(['`:name` must start with `@`'/**m0f809631289d02f8e*/, 'name' => $name]);
        }
    ...
    {
        if ($name[0] !== '@') {
            throw new InvalidFormatException(['`:name` must start with `@`'/**m0f7f21386c79f1518*/, 'name' => $name]);
        }
    ...
    public function resolve($path)
    {
        ...
        if (!isset($this->_aliases[$alias])) {
            throw new InvalidFormatException(['`:alias` is not exists for `:path`'/**m0aac421937afe5850*/, 'alias' => $alias, 'path' => $path]);
        }
    ...
    public function resolveNS($ns)
    {
        ...
        $alias = $parts[0];
        if (!isset($this->_aliases[$alias])) {
            throw new InvalidFormatException(['`:alias` is not exists for `:namespace`'/**m0aac421937afe5850*/, 'alias' => $alias, 'namespace' => $ns]);
        }
